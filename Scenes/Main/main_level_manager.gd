...
# Store the tween so we can cancel it if the player runs in and out really fast!
var current_tween: Tween

func _ready() -> void:
	PlayerGlobals.set_in_menu_stats.emit()
	
	var play_area_walls = play_area.get_node("Hologram").get_children()
...
func _on_play_area_body_entered(body: Node3D) -> void:
	if body.is_in_group("Player"):
		print("fading in")
		fade_all_play_area_walls(false) # false = fade in

func _on_play_area_body_exited(body: Node3D) -> void:
	if body.is_in_group("Player"):
		print("fading out")
		fade_all_play_area_walls(true) # true = fade out

func fade_all_play_area_walls(is_fade_out: bool = false, fade_time: float = 0.5) -> void:
